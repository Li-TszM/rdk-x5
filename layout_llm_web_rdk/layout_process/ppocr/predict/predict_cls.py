...
from process.postprocess_cls import ClsPostProcess


class TextClassifier(PredictBase):
    def __init__(self, conf):
        super(TextClassifier, self).__init__()
        self.cls_image_shape = [int(v) for v in conf.cls_image_shape.split(",")]
        self.cls_batch_num = conf.cls_batch_num
        self.cls_thresh = conf.cls_thresh
        self.postprocess_op = ClsPostProcess(label_list=conf.label_list)

        # 初始化模型
        self.cls_onnx_session = self.get_onnx_session(conf.cls_model_dir, conf.use_gpu)
